...
def check_failed_dockerfile(scripts, lines_required=None):
    npm_bin = which("npm")
    npm_bin_path = (
        subprocess.check_output([npm_bin, "list"]).decode("UTF-8").strip("\n")
        if npm_bin
        else ""
    )
    npm_bin_path_g = (
        subprocess.check_output([npm_bin, "list", "-g"]).decode("UTF-8").strip("\n")
        if npm_bin
        else ""
    )
    lint_bin_name = "dockerfile_lint"
    lint_bin = which(lint_bin_name) or which(
        lint_bin_name, path=npm_bin_path + os.pathsep + npm_bin_path_g
    )
...
def test_main():
    ...
    lines_required = [
        ...
        '&& source /rvm_env.sh && /install"'.format(
            source_py=sources_py, source_js=sources_js
        ),
    ...
    lines_required.append(
        'RUN /bin/bash -c "{source_py} && {source_js} && '
        "source /rvm_env.sh && "
        '/before_install && /install"'.format(
            source_py=sources_py, source_js=sources_js
        ),
    )
    check_failed_dockerfile(
        scripts, lines_required + ["ENV TRAVIS_REPO_SLUG=Vauxoo/travis2docker"]
    )
